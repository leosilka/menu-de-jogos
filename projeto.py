...
while escolha != 5:
    ...
    if escolha == 1: #Aqui está o programa de Adivinhação
        ...
        while r == 'S':
            ...
            nescolhido = int(input('Tente adivinhar o número que estou pensando entre 0 e 10: '))
            if nescolhido == nsorteado:
                print('Parabéns, você acertou o número que estava pensando.')
                acertos += 1
                r = str(input('Deseja continuar jogando? [S/N]')).strip().upper()
            elif nescolhido != nsorteado:
                print('Infelizmente você errou o número qeu estava pensando. O número que pensei foi {}.' .format(nsorteado))
                erros += 1
                r = str(input('Deseja continuar jogando? [S/N]')).strip().upper()
        print('Você jogou {} vezes, teve {} acertos e {} erros.' .format(cont, acertos, erros))
        ...
        while r == 'S':
            ...
            if ia == 0: #IA jogou Pedra
                ...
                if player == 0:
                    ...
                    print('Jogada Inválida!')
                r = str(input('Deseja continuar jogando? [S/N]')).strip().upper()
        print('Você teve {} acertos, {} empates e {} derrotas.' .format(acertos, empates, erros))
        print('Retornando ao menu...')
        sleep(3)
    elif escolha == 5:
        print('Finalizando...')
